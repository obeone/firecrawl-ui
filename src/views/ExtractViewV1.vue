...
if (!clients || clients.version !== 'v1') {
  throw new Error('Extraction API v1 is not available');
}
const extractionApi = clients.extraction as ExtractionApi;
...
const runExtraction = async (): Promise<void> => {
  ...
  try {
    loading.value = true;
    error.value = '';
    const response = await extractionApi.extractData(payload);
