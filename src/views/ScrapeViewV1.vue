...
export default defineComponent({
  ...
  setup() {
    ...
    if (!clients || clients.version !== 'v1') {
      throw new Error('Scraping API v1 is not available');
    }
    const scrapingApi = clients.scraping as ScrapingApi;
    ...
    const handleSubmit = async (): Promise<void> => {
      ...
      try {
        ...
        const response = await scrapingApi.scrapeAndExtractFromUrl(requestPayload);
